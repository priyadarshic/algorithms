 ...
 * 3. Constant Leak Rate: Water "leaks" out of the bucket at a constant rate
 * (e.g., 5 requests per second).
 * 4. Smoothing: This ensures that requests are processed at a stable, uniform
 * rate, regardless of spikes.
 * 
 * Pros:
...
public class LeakyBucketRateLimiter implements RateLimiter {
    ...
    private void leak() {
        ...
        if (leakedAmount > 0) {
            currentWater = (int) Math.max(0, currentWater - leakedAmount);
